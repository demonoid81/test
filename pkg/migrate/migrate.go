...
func NewMigrator(ctx context.Context, conn *pglx.DB, versionTable string, options *MigratorOptions) (m *Migrator, err error) {
...
func NewMigratorEx(ctx context.Context, conn *pglx.DB, versionTable string, opts *MigratorOptions) (m *Migrator, err error) {
...
func acquireAdvisoryLock(ctx context.Context, conn *pglx.DB) error {
	c, err := conn.Pool.Acquire(ctx)
	if err != nil {
		return err
	}
	return crdbpgx.ExecuteTx(ctx, c, pgx.TxOptions{}, func(tx pgx.Tx) (err error) {
		var databaseName string
		err = tx.QueryRow(ctx, "SELECT current_database()").Scan(&databaseName)
		if err != nil {
			return err
		}

		aid, err := generateAdvisoryLockId(databaseName)
		if err != nil {
			return err
		}

		query := "SELECT * FROM " + defaultLockTable + " WHERE lock_id = $1"
		rows, err := tx.Query(ctx, query, aid)
		if err != nil {
			return errors.New(fmt.Sprintf("failed to fetch migration lock: %s (details: %v)", []byte(query), err))
		}
		defer func() {
			rows.Close()
		}()
		...
		query = "INSERT INTO " + defaultLockTable + " (lock_id) VALUES ($1)"
		if _, err := tx.Exec(ctx, query, aid); err != nil {
			return errors.New(fmt.Sprintf("failed to fetch migration lock: %s (details: %v)", []byte(query), err))
		}
		return nil
	})
...
func releaseAdvisoryLock(ctx context.Context, conn *pglx.DB) error {
	var databaseName string
	err := conn.QueryRow(ctx, "SELECT current_database()").Scan(&databaseName)
	if err != nil {
		return err
	}
	...
	query := "DELETE FROM " + defaultLockTable + " WHERE lock_id = $1"
	if _, err := conn.Exec(ctx, query, aid); err != nil {
		...
		return errors.New(fmt.Sprintf("failed to release migration lock: %s (details: %v)", []byte(query), err))
	}
	return nil
}
