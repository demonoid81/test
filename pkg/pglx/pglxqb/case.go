...
// QBuilderBuffer is a helper that allows to write many Sqlizers one by one
// without constant checks for errors that may come from QB
type QBuilderBuffer struct {
	bytes.Buffer
	args []interface{}
	err  error
}

// WriteSql converts QB to SQL strings and writes it to buffer
func (b *QBuilderBuffer) WriteSql(item QB) {
	if b.err != nil {
		return
	}
...
func (b *QBuilderBuffer) ToSql() (string, []interface{}, error) {
