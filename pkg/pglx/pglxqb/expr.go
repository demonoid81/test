...
func Expr(sql string, args ...interface{}) QB {
...
func (ce concatExpr) ToSql() (sql string, args []interface{}, err error) {
	for _, part := range ce {
		...
		default:
			return "", nil, fmt.Errorf("%#v is not a string or QB", part)
		}
	}
	return
}
...
func Alias(expr QB, alias string) aliasExpr {
...
func (b Between) ToSql() (string, []interface{}, error) {
	if b.Field == ""  || b.X == nil || b.Y == nil  {
		return sqlFalse, []interface{}{}, nil
	}
	args := []interface{}{b.X, b.Y}
	sql := fmt.Sprintf("%s BETWEEN ? AND ?", b.Field)
	return sql, args, nil
}
