...
func (r *Resolver) RegionMutation(ctx context.Context, region *models.Region) (*models.Region, error) {
	...
	rows, _, err := region.Mutation(ctx, tx, r.env, nil, columns)
	if err != nil {
		r.env.Logger.Error().Str("module", "medicalBooks").Str("func", "MedicalBookMutation").Err(err).Msg("Error mutation medicalBook")
		return nil, err
	}
	region, err = region.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, tx)
	if err != nil {
	...
	for _, c := range subscriptionsMutateRegionResults.MutateRegionResults[uuid.Nil] {
		c <- region
	}
	return region, err
}

func (r *Resolver) Region(ctx context.Context, region *models.Region) (*models.Region, error) {
	var err error
	sql := pglxqb.Select("regions.*").From("regions")
	result, sql, err := models.SqlGenSelectKeys(region, sql, "regions", 1)
	if err != nil {
	...
	return region.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}

func (r *Resolver) Regions(ctx context.Context, region *models.Region, offset *int, limit *int) ([]*models.Region, error) {
	var err error
	sql := pglxqb.Select("regions.*").From("regions")
	result, sql, err := models.SqlGenSelectKeys(region, sql, "regions", 1)
	if err != nil {
	...
	return region.ParseRows(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}
