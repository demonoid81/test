...
func (r *Resolver) CityDistrictMutation(ctx context.Context, cityDistrict *models.CityDistrict) (*models.CityDistrict, error) {
	...
	rows, _, err := cityDistrict.Mutation(ctx, tx, r.env, nil, columns)
	if err != nil {
		r.env.Logger.Error().Str("module", "medicalBooks").Str("func", "MedicalBookMutation").Err(err).Msg("Error mutation medicalBook")
		return nil, err
	}
	cityDistrict, err = cityDistrict.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, tx)
	if err != nil {
	...
	return cityDistrict, err
}

func (r *Resolver) CityDistrict(ctx context.Context, cityDistrict *models.CityDistrict) (*models.CityDistrict, error) {
	var err error
	sql := pglxqb.Select("city_districts.*").From("city_districts")
	result, sql, err := models.SqlGenSelectKeys(cityDistrict, sql, "city_districts", 1)
	if err != nil {
	...
	return cityDistrict.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}

func (r *Resolver) CityDistricts(ctx context.Context, cityDistrict *models.CityDistrict, offset *int, limit *int) ([]*models.CityDistrict, error) {
	var err error
	sql := pglxqb.Select("city_districts.*").From("city_districts")
	result, sql, err := models.SqlGenSelectKeys(cityDistrict, sql, "city_districts", 1)
	if err != nil {
	...
	return cityDistrict.ParseRows(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}
