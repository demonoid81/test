...
func (r *Resolver) SettlementMutation(ctx context.Context, settlement *models.Settlement) (*models.Settlement, error) {
	...
	rows, _, err := settlement.Mutation(ctx, tx, r.env, nil, columns)
	if err != nil {
		r.env.Logger.Error().Str("module", "medicalBooks").Str("func", "MedicalBookMutation").Err(err).Msg("Error mutation medicalBook")
		return nil, err
	}
	settlement, err = settlement.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, tx)
	if err != nil {
...
func (r *Resolver) Settlement(ctx context.Context, settlement *models.Settlement) (*models.Settlement, error) {
	var err error
	sql := pglxqb.Select("settlements.*").From("settlements")
	result, sql, err := models.SqlGenSelectKeys(settlement, sql, "settlements", 1)
	if err != nil {
	...
	return settlement.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}

func (r *Resolver) Settlements(ctx context.Context, settlement *models.Settlement, offset *int, limit *int) ([]*models.Settlement, error) {
	var err error
	sql := pglxqb.Select("settlements.*").From("settlements")
	result, sql, err := models.SqlGenSelectKeys(settlement, sql, "settlements", 1)
	if err != nil {
	...
	return settlement.ParseRows(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}
