...
func (r *Resolver) CountryMutation(ctx context.Context, country *models.Country) (*models.Country, error) {
	...
	rows, _, err := country.Mutation(ctx, tx, r.env, nil, columns)
	if err != nil {
		r.env.Logger.Error().Str("module", "medicalBooks").Str("func", "MedicalBookMutation").Err(err).Msg("Error mutation medicalBook")
		return nil, err
	}
	country, err = country.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, tx)
	if err != nil {
...
func (r *Resolver) Country(ctx context.Context, country *models.Country) (*models.Country, error) {
	var err error
	sql := pglxqb.Select("countries.*").From("countries")
	result, sql, err := models.SqlGenSelectKeys(country, sql, "countries", 1)
	if err != nil {
	...
	return country.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}

func (r *Resolver) Countries(ctx context.Context, country *models.Country, offset *int, limit *int) ([]*models.Country, error) {
	var err error
	sql := pglxqb.Select("countries.*").From("countries")
	result, sql, err := models.SqlGenSelectKeys(country, sql, "countries", 1)
	if err != nil {
	...
	return country.ParseRows(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}
