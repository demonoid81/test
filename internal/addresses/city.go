...
func (r *Resolver) CityMutation(ctx context.Context, city *models.City) (*models.City, error) {
	...
	rows, _, err := city.Mutation(ctx, tx, r.env, nil, columns)
	if err != nil {
		r.env.Logger.Error().Str("module", "medicalBooks").Str("func", "MedicalBookMutation").Err(err).Msg("Error mutation medicalBook")
		return nil, err
	}
	city, err = city.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, tx)
	if err != nil {
	...
	for _, c := range subscriptionsMutateCityResults.MutateCityResult[uuid.Nil] {
		c <- city
	}
	return city, err
}

func (r *Resolver) City(ctx context.Context, city *models.City) (*models.City, error) {
	var err error
	sql := pglxqb.Select("cities.*").From("cities")
	result, sql, err := models.SqlGenSelectKeys(city, sql, "cities", 1)
	if err != nil {
	...
	return city.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}

func (r *Resolver) Cities(ctx context.Context, city *models.City, offset *int, limit *int) ([]*models.City, error) {
	var err error
	sql := pglxqb.Select("cities.*").From("cities")
	result, sql, err := models.SqlGenSelectKeys(city, sql, "cities", 1)
	if err != nil {
	...
	return city.ParseRows(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}
