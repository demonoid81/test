...
func (r *Resolver) Check(ctx context.Context, job *models.Job, lat *float64, lon *float64, user *models.User) (bool, error) {
	...
	var executor uuid.UUID
	var object uuid.UUID
	if err = pglxqb.Select("uuid_statuses", "uuid_executor", "uuid_object").
		From("jobs").
		Where(pglxqb.Eq{"uuid": job.UUID}).
		RunWith(tx).QueryRow(ctx).
		Scan(&statusesUUID, &executor, &object); err != nil {
	...
	if executor != personUUID {
		// зафиксируем изменений статуса
		statusUUID := uuid.New()
		if _, err = pglxqb.Insert("statuses").
			Columns("uuid", "uuid_job", "status", "lat", "lon", "uuid_person", "uuid_executor").
			Values(statusUUID, job.UUID, models.JobStatusReady, lat, lon, personUUID, personUUID).
		...
		if _, err = pglxqb.Update("jobs").
			Set("status", models.JobStatusReady).
			Set("uuid_executor", personUUID).
	...
	for _, token := range notificationTokens {
		if token != nil {
			text := "Исполнитель подтвердил выход на смену"
