...
func (r *Resolver) CandidateMutate(ctx context.Context, candidate *models.Candidate) (*models.Candidate, error) {
	...
	rows, _, err := candidate.Mutation(ctx, tx, r.env, nil, columns)
	if err != nil {
		r.env.Logger.Error().Str("module", "medicalBooks").Str("func", "MedicalBookMutation").Err(err).Msg("Error mutation medicalBook")
		return nil, err
	}
	candidate, err = candidate.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, tx)
	if err != nil {
...
func (r *Resolver) Candidate(ctx context.Context, candidate *models.Candidate) (*models.Candidate, error) {
	var err error
	sql := pglxqb.Select("candidates.*").From("candidates")
	result, sql, err := models.SqlGenSelectKeys(candidate, sql, "candidates", 1)
	if err != nil {
	...
	return candidate.ParseRow(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}

func (r *Resolver) Candidates(ctx context.Context, candidate *models.Candidate, offset *int, limit *int) ([]*models.Candidate, error) {
	var err error
	sql := pglxqb.Select("candidates.*").From("candidates")
	result, sql, err := models.SqlGenSelectKeys(candidate, sql, "candidates", 1)
	if err != nil {
	...
	return candidate.ParseRows(ctx, r.env, graphql.CollectFieldsCtx(ctx, nil), rows, r.env.Cockroach)
}
